# expenses-tracker-bankrecon

Internal bank reconciliation tool that captures company invoices and personal expense receipts, then automatically matches them against monthly bank statements for auditor-ready output.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "214332408+sfchong72@users.noreply.github.com"
  git config user.name "sfchong72"
  ```
