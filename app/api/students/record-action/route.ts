...
export async function POST(request: Request) {
  ...
  const actionNote = `${action === "archive" ? "Archived" : "Marked for duplicate review"}: ${note}`;
  const preservedRemarks = [studentRes.data.remarks, actionNote].filter(Boolean).join("\n");
  const patch = action === "archive"
    ? { lifecycle_status: "archived", active_status: false, remarks: preservedRemarks }
    : { duplicate_review_status: "possible_duplicate", remarks: preservedRemarks };
