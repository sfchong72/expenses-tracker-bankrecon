...
const isCancelledVoucher = (voucher: Row) => ["cancelled", "voided"].includes(String(voucher.status || ""));
const voucherStatusLabel = (voucher: Row) => isCancelledVoucher(voucher) ? "Void / Cancelled" : String(voucher.status || "").replaceAll("_", " ");
...
export function Phase2Workspace({ mode, billId }: { mode: Mode; billId?: string }) {
  ...
  async function deleteVoucher(id: string) {
    if (!window.confirm("Delete this draft voucher? This removes the draft and its item lines.")) return;
    const res = await fetch("/api/payment-vouchers/delete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ voucherId: id }) });
    const json = await res.json();
    if (!res.ok) setError(json.error || "Draft voucher could not be deleted."); else { setMessage("Draft voucher deleted."); await load(); }
  }

  async function voidVoucher(id: string) {
    const reason = window.prompt("Enter the reason for voiding or cancelling this issued voucher.");
    if (!reason?.trim()) return;
    const res = await fetch("/api/payment-vouchers/void", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ voucherId: id, reason }) });
    const json = await res.json();
    if (!res.ok) setError(json.error || "Voucher could not be cancelled."); else { setMessage("Voucher marked Void / Cancelled and retained for audit trail."); await load(); }
  }
...
function VoucherTable({ rows, items, entities, categories, docs, links, profiles, onIssue, onEdit, onDelete, onVoid }: Row) { return !rows.length ? <div className="empty">No payment vouchers yet.</div> : <table><thead><tr><th>No</th><th>Date</th><th>Payee</th><th>Purpose</th><th>Items</th><th>Total</th><th>Status</th><th /></tr></thead><tbody>{rows.map((v: Row) => <tr key={v.id}><td><Demo row={v} /> {v.voucher_number || "Draft"}</td><td>{v.voucher_date}</td><td>{v.payee}</td><td>{v.purpose}</td><td>{items.filter((i: Row) => i.payment_voucher_id === v.id).map((i: Row) => `${i.description} (${categoryName(i.expense_category_id)})`).join("; ")}</td><td>{money(v.total_amount)}</td><td><span className={`status-pill status-${String(v.status || "").replaceAll("_", "-")}`}>{voucherStatusLabel(v)}</span></td><td>{v.status === "draft" && <><button onClick={() => onEdit(v)}>Edit</button><button className="danger" onClick={() => void onDelete(v.id)}>Delete</button><button onClick={() => void onIssue(v.id)}>Issue Voucher</button></>}{v.status !== "draft" && !isCancelledVoucher(v) && <button className="danger" onClick={() => void onVoid(v.id)}>Void / Cancel Voucher</button>}<button onClick={() => printVoucher(v, items.filter((i: Row) => i.payment_voucher_id === v.id), entities, categories, docs, links, profiles)}>Print</button></td></tr>)}</tbody></table>; function categoryName(id: string) { return categories.find((c: Row) => c.id === id)?.name ?? "-"; } }
function printVoucher(v: Row, voucherItems: Row[], entities: Row[], categories: Row[], docs: Row[], links: Row[], profiles: Row[]) { const entity = entities.find((e) => e.id === v.entity_id); const docList = links.filter((l) => l.linked_record_type === "payment_voucher" && l.linked_record_id === v.id).map((l) => docs.find((d) => d.id === l.document_id)?.original_filename).filter(Boolean).join(", ") || "None"; const rows = voucherItems.map((i) => `<tr><td>${i.description}</td><td>${categories.find((c) => c.id === i.expense_category_id)?.name ?? "-"}</td><td style="text-align:right">${money(i.amount)}</td></tr>`).join(""); const cancelBlock = isCancelledVoucher(v) ? `<div style="margin:12px 0;padding:12px;border:2px solid #b44949;color:#b44949;font-weight:700">VOID / CANCELLED${v.cancellation_reason ? `<br /><span style="font-weight:400">Reason: ${v.cancellation_reason}</span>` : ""}</div>` : ""; const w = window.open("", "_blank"); if (!w) return; w.document.write(`<html><head><title>${v.voucher_number || "Draft Voucher"}</title><style>body{font-family:Arial;padding:24px}table{width:100%;border-collapse:collapse}td,th{border:1px solid #999;padding:8px}@media print{button{display:none}}</style></head><body><h1>Payment Voucher</h1>${cancelBlock}<p><b>Company:</b> ${entity?.legal_name || entity?.short_code || ""}</p><p><b>Voucher:</b> ${v.voucher_number || "Draft"}</p><p><b>Date:</b> ${v.voucher_date}</p><p><b>Payee:</b> ${v.payee}</p><p><b>Payee bank details:</b><br />${(v.payee_bank_details?.notes || "Not provided").replaceAll("\n", "<br />")}</p><p><b>Purpose:</b> ${v.purpose}</p><table><thead><tr><th>Description</th><th>Category</th><th>Amount</th></tr></thead><tbody>${rows}</tbody></table><p><b>Total:</b> ${money(v.total_amount)}</p><p><b>Status:</b> ${voucherStatusLabel(v)}</p><p><b>Payment method:</b> ${v.payment_method || ""}</p><p><b>Bank reference:</b> ${v.bank_reference || ""}</p><p><b>Prepared by:</b> ${profiles.find((profile: Row) => profile.id === v.prepared_by)?.display_name || profiles.find((profile: Row) => profile.id === v.prepared_by)?.email || v.prepared_by || ""}</p><p><b>Remarks:</b> ${v.remarks || ""}</p><p><b>Supporting documents:</b> ${docList}</p><button onclick="window.print()">Print / Save PDF</button></body></html>`); w.document.close(); }
